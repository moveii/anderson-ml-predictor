...
struct AndersonCore
    ...
    function AndersonCore(nbath::Int)
...
c[1, σ] are the anihilators of the impurity site per convention.
"""
function non_interacting_hamiltonian(core::AndersonCore, parameters::AndersonParameters)
...
function hamiltonian(core::AndersonCore, parameters::AndersonParameters)
    # be aware: there is a (small) difference between 0 * operator and no operator
    # to not end up with extremly small numbers instead of 0, do it this way
...
function greens_operators(site::NTuple{2,Int64}, core::AndersonCore)
...
function hybridisation_tau(τ::AbstractVector, parameters::AndersonParameters, β::Real)
    ...
    for i in 1:nbath(parameters)
        sum += (parameters.v[i] .* parameters.v[i]) .* exp.(-τ .* parameters.ε[i]) ./ (1 + exp(-β * parameters.ε[i]))
    end
