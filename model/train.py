...
def load_data(file_path, encoder_input, labels):
...
def validate_mape(model, loader, label_scaler, use_scaling=True, epsilon=1e-8):
    model.eval()
    total_loss = 0
...
def validate_autoregressive_mape(model, val_loader, device, label_scaler, use_scaling=True, epsilon=1e-8):
    model.eval()
    losses = []
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, criterion, config, label_scaler):
...
def plot_losses(train_losses, val_losses, train_mapes, val_mapes):
